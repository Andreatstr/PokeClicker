...
import { getContrastColor, getStatBarColors, getUnknownPokemonColors, getTypeColors } from '../typeColors'
...
describe('getStatBarColors', () => {
  it('should return light mode colors when isDarkMode is false', () => {
    ...
    expect(colors.hp).toEqual({ color: 'bg-red-300', upgradeColor: 'bg-red-600' })
    expect(colors.attack).toEqual({ color: 'bg-orange-300', upgradeColor: 'bg-orange-600' })
    expect(colors.defense).toEqual({ color: 'bg-blue-300', upgradeColor: 'bg-blue-600' })
    expect(colors.spAttack).toEqual({ color: 'bg-purple-300', upgradeColor: 'bg-purple-600' })
    expect(colors.spDefense).toEqual({ color: 'bg-yellow-300', upgradeColor: 'bg-yellow-600' })
    expect(colors.speed).toEqual({ color: 'bg-pink-300', upgradeColor: 'bg-pink-600' })
  })

  it('should return dark mode colors when isDarkMode is true', () => {
    const colors = getStatBarColors(true)
    
    expect(colors.hp).toEqual({ color: 'bg-red-500/60', upgradeColor: 'bg-red-400/80' })
    expect(colors.attack).toEqual({ color: 'bg-orange-500/60', upgradeColor: 'bg-orange-400/80' })
    expect(colors.defense).toEqual({ color: 'bg-blue-500/60', upgradeColor: 'bg-blue-400/80' })
    expect(colors.spAttack).toEqual({ color: 'bg-purple-500/60', upgradeColor: 'bg-purple-400/80' })
    expect(colors.spDefense).toEqual({ color: 'bg-yellow-500/60', upgradeColor: 'bg-yellow-400/80' })
    expect(colors.speed).toEqual({ color: 'bg-pink-500/60', upgradeColor: 'bg-pink-400/80' })
  })
})
...
describe('getTypeColors', () => {
  ...
  it('should return normal colors when type is undefined', () => {
    const lightColors = getTypeColors(undefined as any, false)
    const darkColors = getTypeColors(undefined as any, true)
  ...
  it('should have different colors for light and dark modes', () => {
    pokemonTypes.forEach(type => {
      ...
      const isDifferent = lightColors.badge !== darkColors.badge || 
  ...
  it('should return specific colors for known types', () => {
    ...
    expect(fireDark.cardBg).toBe('bg-gradient-to-br from-red-600/60 to-red-700/50')
  })
