...
describe('PokeClicker component', () => {
  ...
  it('should render game interface', () => {
    ...
    expect(screen.getByRole('button', { name: /click charizard to earn rare candy/i })).toBeInTheDocument()
  })
  ...
  it('should show unauthenticated message when not logged in', () => {
    ...
    expect(screen.getByText('You need to log in to play the clicker game and save your progress.')).toBeInTheDocument()
  })
  ...
  it('should handle candy clicking when authenticated', async () => {
    ...
    const clickButton = screen.getByRole('button', { name: /click charizard to earn rare candy/i })
  ...
  it('should handle stat upgrades', async () => {
    ...
    mockAuth.user = mockUser
    mockAuth.isAuthenticated = true
    mockUpgradeStat.mockResolvedValue({ ...mockUser, stats: { ...mockUser.stats, hp: 101 } })
