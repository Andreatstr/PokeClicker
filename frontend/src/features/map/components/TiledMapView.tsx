...
// Demo button styles (non-interactive buttons in "How to play" section)
const DEMO_BUTTON_STYLE: React.CSSProperties & {
  '--custom-inner-border-color'?: string;
} = {
  borderColor: '#2a2a3e',
...
export function TiledMapView(props: TiledMapViewProps) {
  ...
  return (
    <>
      ...
      {teleportLocation && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-[60] pointer-events-none animate-fade-in">
          <div
      ...
      <div className="absolute bottom-3 left-2 z-20">
        <button
          ...
          className="flex items-center gap-1 h-10 border-2 border-black px-3 disabled:opacity-50 disabled:cursor-not-allowed"
          title={
            isTeleporting
              ? 'Teleporting...'
              : teleportCooldown > 0
      ...
      {/* World Info Modal */}
      <Dialog open={showWorldInfo} onClose={handleCloseWorldInfo}>
        <DialogBody>
