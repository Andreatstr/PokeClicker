### Issue #P4-6: Add Sort Direction (Ascending/Descending) to Filter Controls

**Branch:** `feat/sort-direction`

**Priority:** 🟡 HIGH - UX improvement

**Problem:** Users can sort by ID, Name, or Type, but cannot control sort direction (ascending vs descending). Sort direction should be integrated into the filter controls alongside existing sort options.

**Current State:**
```tsx
// FilterControls has sort by field/type, but no direction control
<FilterModal>
  {/* Search */}
  {/* Region filters */}
  {/* Type filters */}
  {/* Sort by: ID/Name/Type */} {/* ❌ No ascending/descending option */}
</FilterModal>
```

**Desired State:**
```tsx
// Add sort direction option in the same filter modal
<FilterModal>
  {/* Search */}
  {/* Region filters */}
  {/* Type filters */}
  {/* Sort by: ID/Name/Type */}
  {/* Sort direction: Ascending/Descending */} {/* ✅ New option */}
</FilterModal>
```
...
```tsx
// FilterControls.tsx - add sort direction control

export function FilterControls({
  sortBy,
  sortDirection, // NEW
  onSortChange,
  onSortDirectionChange, // NEW
}: Props) {
  return (
...
**Backend Changes:**
```ts
// resolvers.ts - support sort direction in pokemon query
...
async pokemon(_: any, args: PokemonQueryArgs, { db }: Context) {
  ...
  let sort: any = {};
  if (sortBy === 'id') {
    sort = { id: sortDirection === 'asc' ? 1 : -1 };
  } else if (sortBy === 'name') {
    sort = { name: sortDirection === 'asc' ? 1 : -1 };
  } else if (sortBy === 'type') {
    sort = { types: sortDirection === 'asc' ? 1 : -1 };
  }

  // Apply sort to query
  const pokemon = await collection.find(query).sort(sort).skip(offset).limit(limit).toArray();
...
**GraphQL Schema:**
```graphql
# schema.ts - add sortDirection parameter
enum SortDirection {
  asc
  desc
}
...
- [ ] Sort by ID descending (151, 150, 149...)
- [ ] Sort by Name ascending (A-Z)
- [ ] Sort by Name descending (Z-A)
- [ ] Sort direction persists when changing sort field
- [ ] Both buttons have 44x44px minimum hit area
- [ ] Keyboard accessible (Tab navigation)

**Acceptance:**
- [ ] Sort direction controls added to filter modal
- [ ] Ascending/Descending buttons toggle correctly
- [ ] Backend applies correct sort order
- [ ] Both controls have 44x44px minimum hit area
- [ ] Responsive design works on mobile
- [ ] Clear visual indication of active direction

**Files:**
- `frontend/src/features/pokedex/components/FilterControls.tsx`
- `frontend/src/App.tsx` (add sortDirection state)
- `backend/src/schema.ts` (add SortDirection enum)
- `backend/src/resolvers.ts` (handle sortDirection parameter)
- `frontend/src/lib/graphql/types.ts` (TypeScript types)
